...
async function loadState() {
  ...
  try {
    const cm = await window.settings.getConsoleMode?.();
    if (typeof cm === 'boolean') document.getElementById('console-toggle').checked = cm;
  } catch {}
  try {
  ...
  try {
    const cs = await window.settings.getCursorSensitivity?.();
    if (typeof cs === 'number') {
      document.getElementById('cursor-slider').value = cs;
      document.getElementById('cursor-label').textContent = cs;
    }
  } catch {}
  try {
...
// CONSOLE MODE
document.getElementById('console-toggle').addEventListener('change', async e => {
  try { await window.settings.setConsoleMode(e.target.checked); }
  catch { e.target.checked = !e.target.checked; }
});
...
// CURSOR SENSITIVITY
document.getElementById('cursor-slider').addEventListener('input', e => {
  document.getElementById('cursor-label').textContent = e.target.value;
});
document.getElementById('cursor-slider').addEventListener('change', async e => {
  const v = Math.min(Math.max(Number(e.target.value), 10), 100);
  try {
    const r = await window.settings.setCursorSensitivity(v);
    if (r?.success && typeof r.value === 'number') {
      document.getElementById('cursor-slider').value = r.value;
      document.getElementById('cursor-label').textContent = r.value;
    }
  } catch {}
});
