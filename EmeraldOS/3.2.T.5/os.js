...
/* =========================================================
   TESTOS 3.2.T.5 DEVELOPER EDITION PATCH
...
(function () {
        ...
        window.openWindow("Build Inspector", `
            <div style="padding:10px;">
                ...
                <div class="inset-panel">
                    <b>Build:</b> ${safeHTML(build.displayName || "TestOS 3.2.T.5")}<br>
                    <b>Version:</b> ${safeHTML(build.version || "3.2.T.5")}<br>
    ...
    /* -----------------------------------------------------
       TESTOS 3.2.T.5 ADVANCED DEVELOPER PATCH
       - More apps
       - Expanded terminal
       - HKEY-style registry
    ...
    const DEFAULT_TESTOS_REGISTRY = {
        ...
        "HKEY_TESTOS\\SOFTWARE\\TestOS\\CurrentVersion": "3.2.T.5",
    ...
    window.runCommand = async function(cmdLine = "") {
        ...
        try {
            ...
            if (command === "help") {
                ...
                result = "TestOS 3.2.T.5 Desktop Folders Suite Build";
            }
    ...
    window.openAudioNotes = function () {
        window.openWindow("Audio Notes", `<div class="app-panel"><h3>Audio Notes</h3><p>Audio note recorder placeholder for TestOS 3.2.T.5.</p><button onclick="notify('Audio Notes','Recorder initialized.',2500,'info')">Initialize Recorder</button></div>`, "audioNotes");
    };
    ...
    window.openScriptLab = function () {
        window.openWindow("Script Lab", `<div class="app-panel script-lab"><h3>Script Lab</h3><textarea id="script_lab_code" spellcheck="false">return 'Hello from TestOS 3.2.T.5';</textarea><button onclick="runScriptLab()">Run</button><pre id="script_lab_output"></pre></div>`, "scriptLab");
    };
...
(function () {
    ...
    function installT4Apps() {
        ...
        window.openHelpCenter = () => simpleWindow("Help Center", `<h3>Help Center</h3><div class="inset-panel">Welcome to TestOS 3.2.T.5. Applications are now organized into desktop folders. Double-click a folder to open its apps.</div>`, "helpCenter");
        ...
        window.openLicenseManager = () => simpleWindow("License Manager", `<h3>License Manager</h3><div class="inset-panel"><b>Active edition:</b> ${safeT4(localStorage.getItem('testos_edition_name') || 'TestOS Economy')}<br><b>Build:</b> TestOS 3.2.T.5</div>`, "licenseManager");
    ...
    function installRegistryT4() {
        try {
            if (window.TestOSRegistry?.set) {
                ...
                window.TestOSRegistry.set("HKEY_LOCAL_MACHINE\\System\\Build\\Version", "3.2.T.5");
    ...
    function installTerminalCommandsT4() {
        ...
        const wrapped = async function(cmdLine = "") {
            ...
            if (command === "help") {
                ...
                result = "TestOS 3.2.T.5 adds true desktop app folders, consolidated app suites, HTML/Markdown/CSV viewers, registry defaults, and Firebase desktop layout sync.";
                ...
                result = "Package: " + safeT4(parts[0] || "unknown") + "<br>Status: installed<br>Build: 3.2.T.5";
            }
            if (result !== null) {
...
(function () {
    ...
    const BUILD_LABEL_T5 = "TestOS 3.2.T.5";
    const DESKTOP_POS_KEY_T5 = "testos_desktop_positions_v5";
    ...
    function installT5Apps() {
        ...
        registerAppT5("about", Object.assign(window.APPS.about || {}, { name: "About TestOS", icon: "i", edition: "economy", category: "essential", hiddenStandalone: false }));
    ...
    window.openHTMLViewerT5 = function () {
        const sample = "<h1>TestOS HTML Viewer</h1><p>Edit HTML on the left and press Preview.</p>";
    ...
    window.openSystemProfilerT5 = function () {
        simpleWindowT5("System Profiler", `
            ...
            <div class="inset-panel">
                ...
                <b>Edition:</b> ${safeT5(localStorage.getItem("testos_edition_name") || "TestOS Economy")}<br>
    ...
    window.openClipboardBoardT5 = function () {
        simpleWindowT5("Clipboard Board", `
            ...
            <textarea id="clipBoardT5" class="viewer-textarea" style="height:220px" placeholder="Store temporary text here.">${safeT5(localStorage.getItem("testos_clipboard_board") || "")}</textarea>
            <br><button onclick="localStorage.setItem('testos_clipboard_board',document.getElementById('clipBoardT5').value);notify('Clipboard','Saved.',2500,'success')">Save</button>
        `, "clipboardBoard");
    };
    ...
    function injectStylesT5() {
        if (document.getElementById("testos32t5-styles")) return;
        const style = document.createElement("style");
        style.id = "testos32t5-styles";
        style.textContent = `
    ...
    function setRegistryDefaultsT5() {
        try {
            window.TestOSRegistry?.set?.("HKEY_LOCAL_MACHINE\\System\\Build\\Version", "3.2.T.5");
            window.TestOSRegistry?.set?.("HKEY_CURRENT_USER\\Software\\TestOS\\Explorer\\DesktopMode", "FoldersOnly");
            window.TestOSRegistry?.set?.("HKEY_LOCAL_MACHINE\\Software\\TestOS\\Applications\\Consolidation", "SuiteAppsEnabled");
            window.TestOSRegistry?.set?.("HKEY_CLASSES_ROOT\\.html\\DefaultApp", "HTML Viewer");
            window.TestOSRegistry?.set?.("HKEY_CLASSES_ROOT\\.md\\DefaultApp", "Markdown Viewer");
            window.TestOSRegistry?.set?.("HKEY_CLASSES_ROOT\\.csv\\DefaultApp", "CSV Viewer");
        } catch (err) {
    ...
    function initT5() {
        if (window.__testosT5Loaded) return;
        window.__testosT5Loaded = true;
        injectStylesT5();
        installT5Apps();
        ...
        renderDesktopFoldersT5();
        renderStartFoldersT5();
        window.notify?.("TestOS", "3.2.T.5 loaded with desktop folders and app suites.", 3200, "success");
    }

    window.TESTOS_APP_CATEGORIES = APP_CATEGORIES_T5;
