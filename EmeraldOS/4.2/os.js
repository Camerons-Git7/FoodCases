...
(function () {
        ...
        window.openWindow("Build Inspector", `
            <div style="padding:10px;">
                ...
                <div class="inset-panel">
                    <b>Build:</b> ${safeHTML(build.displayName || "EmeraldOS 4.2")}<br>
    ...
    window.runCommand = async function(cmdLine = "") {
        ...
        try {
            ...
            if (command === "help") {
                ...
                result = "EmeraldOS 4.2 Desktop Folders Suite Build";
            }
    ...
    window.openAudioNotes = function () {
        window.openWindow("Audio Notes", `<div class="app-panel"><h3>Audio Notes</h3><p>Audio note recorder placeholder for EmeraldOS 4.2.</p><button onclick="notify('Audio Notes','Recorder initialized.',2500,'info')">Initialize Recorder</button></div>`, "audioNotes");
    };
    ...
    window.openScriptLab = function () {
        window.openWindow("Script Lab", `<div class="app-panel script-lab"><h3>Script Lab</h3><textarea id="script_lab_code" spellcheck="false">return 'Hello from EmeraldOS 4.2';</textarea><button onclick="runScriptLab()">Run</button><pre id="script_lab_output"></pre></div>`, "scriptLab");
    };
...
(function () {
    ...
    function installT4Apps() {
        ...
        window.openHelpCenter = () => simpleWindow("Help Center", `<h3>Help Center</h3><div class="inset-panel">Welcome to EmeraldOS 4.2. Applications are now organized into desktop folders. Double-click a folder to open its apps.</div>`, "helpCenter");
        ...
        window.openLicenseManager = () => simpleWindow("License Manager", `<h3>License Manager</h3><div class="inset-panel"><b>Active edition:</b> ${safeT4(localStorage.getItem('40_edition_name') || 'EmeraldOS Virtue')}<br><b>Build:</b> EmeraldOS 4.2</div>`, "licenseManager");
    ...
    function installTerminalCommandsT4() {
        ...
        const wrapped = async function(cmdLine = "") {
            ...
            if (command === "help") {
                ...
                result = "EmeraldOS 4.2 adds true desktop app folders, consolidated app suites, HTML/Markdown/CSV viewers, registry defaults, and Firebase desktop layout sync.";
...
(function () {
    ...
    const BUILD_LABEL_T5 = "EmeraldOS 4.2";
...
(function () {
    ...
    window.openReleaseNotes41 = function () {
        simple41("Release Notes", `
            <h3>EmeraldOS 4.2 Pinning & Control Update</h3>
            <ul>
    ...
    function wrapTerminal41() {
        ...
        const wrapped = async function (cmdLine) {
            ...
            if (command === "storage.stats") {
                ...
                result = "EmeraldOS 4.2 Pinning & Control Update";
...
(function () {
    ...
    const BUILD_LABEL_42 = "EmeraldOS 4.2";
    ...
    async function scheduleCloudSettings42() {
        ...
        window.__emerald42CloudSettingsTimer = setTimeout(async () => {
            try {
                ...
                console.warn("EmeraldOS 4.2 cloud settings sync skipped:", err);
            }
        }, 600);
    }
    ...
    function installTerminalCommands42() {
        ...
        const wrapped = async function (cmdLine = "") {
            ...
            else if (cmd === "version" || cmd === "build") { result = "EmeraldOS 4.2 Pinning & Control Update"; }
            if (result !== null) {
                const output = document.getElementById("terminal_output");
                if (output) {
    ...
    function init42() {
        ...
        window.notify?.("EmeraldOS 4.2", "Pinning, Win95 menu, and control apps loaded.", 3500, "success");
    }
