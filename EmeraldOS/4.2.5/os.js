...
(function () {
        ...
        window.openWindow("Build Inspector", `
            <div style="padding:10px;">
                ...
                <div class="inset-panel">
                    <b>Build:</b> ${safeHTML(build.displayName || "EmeraldOS 4.2.5")}<br>
                    <b>Version:</b> ${safeHTML(build.version || "4.2")}<br>
    ...
    const DEFAULT_EMERALDOS_REGISTRY = {
        ...
        "HKEY_EMERALDOS\\SOFTWARE\\EmeraldOS\\CurrentVersion": "4.2",
    ...
    window.runCommand = async function(cmdLine = "") {
        ...
        try {
            ...
            if (command === "help") {
                ...
                result = "EmeraldOS 4.2.5 Desktop Folders Suite Build";
            }
    ...
    window.openAudioNotes = function () {
        window.openWindow("Audio Notes", `<div class="app-panel"><h3>Audio Notes</h3><p>Audio note recorder placeholder for EmeraldOS 4.2.5.</p><button onclick="notify('Audio Notes','Recorder initialized.',2500,'info')">Initialize Recorder</button></div>`, "audioNotes");
    };
    ...
    window.openScriptLab = function () {
        window.openWindow("Script Lab", `<div class="app-panel script-lab"><h3>Script Lab</h3><textarea id="script_lab_code" spellcheck="false">return 'Hello from EmeraldOS 4.2.5';</textarea><button onclick="runScriptLab()">Run</button><pre id="script_lab_output"></pre></div>`, "scriptLab");
    };
...
(function () {
    ...
    function installT4Apps() {
        ...
        window.openHelpCenter = () => simpleWindow("Help Center", `<h3>Help Center</h3><div class="inset-panel">Welcome to EmeraldOS 4.2.5. Applications are now organized into desktop folders. Double-click a folder to open its apps.</div>`, "helpCenter");
        ...
        window.openLicenseManager = () => simpleWindow("License Manager", `<h3>License Manager</h3><div class="inset-panel"><b>Active edition:</b> ${safeT4(localStorage.getItem('40_edition_name') || 'EmeraldOS Virtue')}<br><b>Build:</b> EmeraldOS 4.2.5</div>`, "licenseManager");
    ...
    function installRegistryT4() {
        try {
            if (window.EmeraldOSRegistry?.set) {
                ...
                window.EmeraldOSRegistry.set("HKEY_LOCAL_MACHINE\\System\\Build\\Version", "4.2");
    ...
    function installTerminalCommandsT4() {
        ...
        const wrapped = async function(cmdLine = "") {
            ...
            if (command === "help") {
                ...
                result = "EmeraldOS 4.2.5 adds true desktop app folders, consolidated app suites, HTML/Markdown/CSV viewers, registry defaults, and Firebase desktop layout sync.";
    ...
    function initT4() {
        ...
        /* 4.2.5 hotfix: old T4 context menu disabled; final Win95 menu owns desktop right-click. */
        renderDesktopT4();
        renderStartMenuT4();
    }
...
(function () {
    ...
    const BUILD_LABEL_T5 = "EmeraldOS 4.2.5";
    ...
    function initT5() {
        ...
        /* 4.2.5 hotfix: old T5 context menu disabled; final Win95 menu owns desktop right-click. */
...
(function () {
    ...
    window.openReleaseNotes41 = function () {
        simple41("Release Notes", `
            <h3>EmeraldOS 4.2.5.5 Context Menu & Pinning Fix</h3>
            <ul>
    ...
    function installApps41() {
        register41("releaseNotes41", { name: "Release Notes", icon: "4.2", edition: "economy", category: "essential", launch: () => window.openReleaseNotes41() });
    ...
    function wrapTerminal41() {
        ...
        const wrapped = async function (cmdLine) {
            ...
            if (command === "storage.stats") {
                ...
                result = "EmeraldOS 4.2.5.5 Context Menu & Pinning Fix";
...
(function () {
    ...
    const BUILD_LABEL_42 = "EmeraldOS 4.2.5";
    ...
    async function scheduleCloudSettings42() {
        ...
        window.__emerald42CloudSettingsTimer = setTimeout(async () => {
            try {
                ...
                console.warn("EmeraldOS 4.2.5 cloud settings sync skipped:", err);
            }
        }, 600);
    }
    ...
    function installTerminalCommands42() {
        ...
        const wrapped = async function (cmdLine = "") {
            ...
            else if (cmd === "version" || cmd === "build") { result = "EmeraldOS 4.2.5.5 Context Menu & Pinning Fix"; }
            if (result !== null) {
                const output = document.getElementById("terminal_output");
                if (output) {
    ...
    function init42() {
        ...
        try { window.EmeraldOSRegistry?.set?.("HKEY_LOCAL_MACHINE\\System\\Build\\Version", "4.2"); } catch {}
        ...
        window.notify?.("EmeraldOS 4.2.5", "Pinning, Win95 menu, and control apps loaded.", 3500, "success");
    }
...
(function () {
    ...
    const BUILD_425 = "EmeraldOS 4.2.5";
    const PIN_START_KEY = "40_pinned_start_apps";
    const PIN_DESKTOP_KEY = "40_pinned_desktop_apps";
    const PIN_TASKBAR_KEY = "40_pinned_taskbar_apps";
    const PIN_POS_KEY = "40_desktop_pin_positions";
    ...
    function makeMovableIcon425(options) {
        ...
        const el = document.createElement("div");
        el.className = "desktop-item emerald425-desktop-item " + (options.extraClass || "");
    ...
    function renderDesktop425() {
        ...
        getVisibleCategories425().forEach(category => {
            makeMovableIcon425({
                ...
                index: index++,
                extraClass: "emerald425-folder-icon",
                open: () => {
        ...
        Object.entries(window.fileSystem?.files || {}).forEach(([id, file]) => {
            ...
            makeMovableIcon425({
                ...
                index: index++,
                fileId: id,
                extraClass: "emerald425-file-icon",
    ...
    function renderDesktopPins425(startIndex = 0) {
        document.querySelectorAll(".emerald425-pinned-desktop").forEach(el => el.remove());
        getPins425(PIN_DESKTOP_KEY).forEach((appId, i) => {
            ...
            makeMovableIcon425({
                ...
                appId,
                index: startIndex + i,
                extraClass: "emerald425-pinned-desktop desktop-pinned-app",
    ...
    function renderTaskbarPins425() {
        ...
        getPins425(PIN_TASKBAR_KEY).forEach(appId => {
            ...
            if (!app) return;
            const btn = document.createElement("button");
            btn.className = "taskbar-pinned-btn emerald425-taskbar-pin";
    ...
    function renderStartPins425() {
        ...
        pins.forEach(appId => {
            ...
            if (!app) return;
            const item = document.createElement("div");
            item.className = "start-item start-pinned-item emerald425-start-pin";
            item.dataset.appId = appId;
    ...
    function showWin95Menu425(x, y, items) {
        ...
        window.__emerald425MenuActions = [];
        menu.className = "show win95-context-menu emerald425-context-menu";
        menu.style.display = "block";
        menu.innerHTML = items.map((item, index) => {
            if (item.sep) return `<div class="context-separator"></div>`;
            const actionIndex = window.__emerald425MenuActions.push(item.action || function(){}) - 1;
            const arrow = item.children ? `<span class="context-arrow">▶</span>` : "";
        ...
        menu.querySelectorAll(".context-item[data-action-index]").forEach(el => {
            el.onclick = e => {
                ...
                hideContextMenu425();
                window.__emerald425MenuActions?.[idx]?.();
            };
        });
    }
    ...
    function installDesktopContextMenu425() {
        const desktop = document.getElementById("desktop");
        if (!desktop || desktop.__emerald425ContextInstalled) return;
        desktop.__emerald425ContextInstalled = true;
    ...
    async function scheduleCloudSettings425() {
        clearTimeout(window.__emerald425CloudSettingsTimer);
        window.__emerald425CloudSettingsTimer = setTimeout(async () => {
            try {
                await window.saveUserSettings?.({
                ...
                console.warn("EmeraldOS 4.2.5 layout sync skipped:", err);
            }
        }, 650);
    }
    ...
    function patchTerminal425() {
        ...
        const wrapped = async function (cmdLine = "") {
            ...
            const cmd = (parts.shift() || "").toLowerCase();
            let result = null;
            if (cmd === "version" || cmd === "build") result = "EmeraldOS 4.2.5 Context Menu & Pinning Fix";
    ...
    function injectStyles425() {
        if (document.getElementById("emerald425-styles")) return;
        const style = document.createElement("style");
        style.id = "emerald425-styles";
        style.textContent = `
            #context-menu.win95-context-menu,
            .win95-context-menu,
            .emerald425-context-menu {
            ...
            .emerald425-context-menu.show { display: block !important; }
            .win95-context-menu .context-item,
            .emerald425-context-menu .context-item {
            ...
            .emerald425-context-menu .context-item:hover {
                background: #000080 !important;
                color: #fff !important;
            }
            .win95-context-menu .context-separator,
            .emerald425-context-menu .context-separator {
            ...
            .emerald425-desktop-item:hover,
            .emerald425-desktop-item.selected { background:#000080; border:1px dotted #fff; }
            .emerald425-desktop-item.dragging { opacity:.82; z-index:99999; }
        `;
        document.head.appendChild(style);
    }

    function patchBuildLabels425() {
        try { document.title = document.title.replace("4.2", "4.2.5"); } catch {}
        try { window.EmeraldOSRegistry?.set?.("HKEY_LOCAL_MACHINE\\System\\Build\\Version", "4.2.5"); } catch {}
    ...
    function init425() {
        if (window.__emerald425InitDone) return;
        window.__emerald425InitDone = true;
        injectStyles425();
        ...
        renderTaskbarPins425();
        renderStartPins425();
        window.notify?.("EmeraldOS 4.2.5", "Context menu, desktop pins, and Win95 notifications fixed.", 3500, "success");
    }
...
/* =========================================================
   EMERALDOS 4.2.5 RIGHT-CLICK HOTFIX
...
(function () {
    ...
    function installFinalContextMenu() {
        const desktop = document.getElementById("desktop");
        if (!desktop || desktop.__emerald425FinalContextInstalled) return;
        desktop.__emerald425FinalContextInstalled = true;
