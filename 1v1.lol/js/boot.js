// Extracted from index.html (4.46 Unity boot + config block). Loaded in-place, synchronously, after WebGL.loader.js.
    ...
    // further callback) â€” which made the old bar sit dead at "090%". We animate the bar toward the real
    ...
    function _renderProgress(t) {
      ...
      if (stageEl) stageEl.textContent = _loadDone ? 'Ready' : (_realP < 0.85 ? 'Downloading gameâ€¦' : 'Initializingâ€¦');
    ...
    // defined this stub; the 4.46 page rewrite under the new loader dropped it â†’
    ...
    // Fullscreen toggle icon (top-right of the game frame) â€” ported from the 4.23 / 1v1lolreloaded.com page.
    ...
    createUnityInstance(canvas, {
      ...
      // === SENDMESSAGE TRACE === wrap unityInstance.SendMessage to log every JSâ†’C# call.
      // Together with [FETCH-TRACE] + [LSM] probe this gives complete visibility of what
      // our bridge does during boot. No wasm/client patches â€” pure JS interceptor.
      try {
        ...
        unityInstance.SendMessage = function(go, method, value) {
          ...
          var vshort = v.length > 80 ? v.slice(0,80) + 'â€¦(' + v.length + 'b)' : v;
      ...
      // stall â€” if RESOLVE fires promptly in foreground, the body IS delivered and the stall is
      ...
      // Live HEAP read each poll (views detach on memory growth). Pure read-only â€” cannot perturb.
      // 2026-05-31: enabled INDEPENDENTLY of _DIAG_OFF (own flag _ADDRPROBE) â€” this is read-only
      ...
      if (window._ADDRPROBE !== false) try {
        ...
        // metadata-class cells: [0]=AddressablesHandler @0x9AB3D0 (CORRECTED 2026-05-31 â€” the old
      ...
      // at LSM+0x44 (NOT a fixed 5-bool gate â€” that model was wrong for 4.46). The boot coroutine's
      ...
      // This is the decisive observability for splash-hang root cause â€” pure read-only setInterval,
      ...
      if (window._LSMPROBE !== false) try {
        ...
        var lp_iv = setInterval(function(){
          ...
          try {
            ...
            (function(){
              ...
              // render, appear after one nav. Coins gate on ServerUser!=null (Ghidra PKOEAIPHLOC) â†’ guest=
              // ServerUser-null is CANONICAL. So track when guest user-state SETTLES vs menu-show: if it
              // settles AFTER the menu scene Awakes, the panels pull stale (blank) â†’ timing root; if before,
              ...
              // IsSocialLoggedIn(bool). Delegate ptr CHANGES on each (un)subscribe â†’ reveals subscribe order.
            ...
            // il2cpp lazy-init GUARD bytes. Each method sets its own guard=1 the FIRST time it runs â†’ tells
            ...
            // (JNMLDJDAKFN: show only if this==0). DECISIVE: Awake=0 â†’ widget GameObject NOT in our menu scene
            // (B, no fix via config); gate=1 but content=0 â†’ gate returned 0 â†’ hidden by a runtime condition
            ...
            (function(){
              ...
              // (+4,+8,+0xc,+0x10,+0x14). 1=valid heap ptr(loaded), 0=null(not loaded). If all 0 â†’ the daily-
              // skins data never loaded â†’ per-slot reader has nothing to iterate â†’ productRead=0.
              ...
              // instance-field byte: cell â†’ static_fields â†’ [SF+0]=singleton instance â†’ byte @inst+off.
              ...
              // IL2CPP array length: cellâ†’SFâ†’[SF+fldoff]=objâ†’[obj+arroff]=Il2CppArrayâ†’max_length@+0xc.
              ...
              // + IsConnected (NetworkManager.FFPPKCLILNI) breakdown â€” the gate returns this & it's the
              ...
              // fails. offlineMgr+0x28!=0 â†’ OfflineMode set (offline-fallback). NM+4!=0 â†’ disconnected.
            ...
            //   +0x71 MDMKOIMILNP, +0x72 BGMOELIGNIF. When ALL set â†’ LoadingFinished. A byte still 0
            ...
            // FULL _did* bool block â€” AUTHORITATIVE field map from 4.46 il2cpp.h LoadingScreenManager_Fields,
            // CONFIRMED by Ghidra setter bodies: PNLDCHECEMNâ†’+0x6c=_didGetPlayerInfo, BJLDJGDKLGNâ†’+0x6d=
            // _didGetChallengesInfo, CCNOLPPJFJAâ†’+0x6f=_didConnectToPhoton, MDMKOIMILNPâ†’+0x71=
            // _didFinishAddressableDownload, BGMOELIGNIFâ†’+0x72=_didInitializeLocalization.
            ...
            // old array had a phantom +0x69 bool that shifted EVERY _did label down by one offset â€” the root
            ...
            // BoolToCheck names the exact _did* field this job's WaitUntil blocks on â†’ the FIRST job whose
            ...
            window.__lsmInsts.forEach(function(inst){
              ...
              // GetPlayerFullData â€” never called on boot. Write the byte directly to confirm it is the
              // ONLY remaining gate (splash should fill 10%â†’100% and clear). NOT the ship fix. Validity-
              ...
              if (window._FORCE_PLAYERINFO === true) {
                ...
                // per job. If the bar advances â†’ a missing flag was the gate. If it STAYS at 10% with every
                // flag forced on every instance â†’ the loop is NOT reading these bytes (reflection field-name
                // mismatch, or loop runs on a non-located instance) â†’ the real gate is elsewhere.
              ...
              // DECISIVE: realProg=1.0 & fill frozen at 0.1 across beats â†’ bar-fill not pumping (Time/loop);
              //          realProg<1.0 â†’ a job is stuck (BLOCKER names it). Watch fill change between beats.
              ...
              if (jobs && jobs.length){
                jobStr=jobs.map(function(j){return (j.name.charAt(0)==='_'?j.name.slice(4):j.name)+(j.set===1?'âœ“':(j.set===0?'âœ—':'?'));}).join(' ');
              ...
              var isLoop = (njobs>0 && hasCg) ? ' â—€LOOP' : '';
        ...
        console.log('%c[LSMPROBE] installed â€” watching LoadingScreenManager wait-list (LSM+0x44)', 'color:#0bf');
      ...
      if (window._TRPROBE !== false && (typeof localStorage==='undefined' || localStorage.TRPROBE!=='0')) try {
        ...
        var tr_iv = setInterval(function(){
          ...
          try {
            ...
            // â˜… CHAIFNKIJCC.tiers.Count = List._size @ +0xc (IL2CPP List<T>: klass4+mon4+items4+_size4)
        ...
        console.log('%c[TRPROBE] installed â€” watching TrophyRoad active-season gate (EEHFGFMDCJI)','color:#0bf');
      ...
      if (window._TRSCAN !== false && (typeof localStorage==='undefined' || localStorage.TRSCAN!=='0')) try {
        ...
        var trs_iv = setInterval(function(){
          ...
          try {
            ...
            if (!trs_klass){
              ...
              if (nm!==TRS_NAME){ console.warn('%c[TRSCAN] klass name mismatch ("'+nm+'") â€” build/offsets differ, abort','color:#f44'); clearInterval(trs_iv); return; }
              trs_klass = sk;
              console.log('%c[TRSCAN] TrophyRoadScreen klass=0x'+sk.toString(16)+' (via <>c.declaringType) â€” scanning heap for live instance','color:#0bf');
            }
            ...
            var verdict = (cached===0) ? 'R1: instance exists but m_CachedPtr=0 (Unity-destroyed) â€” populate never rendered'
                        : (poolSize<=0) ? 'R1: live instance but _trophyRoadTierItems._size='+poolSize+' (empty) and no AOOR => DMJELOOMNMM/foreach never ran'
                        : 'R2: pool _size='+poolSize+' tiles created â€” invisible => layout/prefab geometry';
        ...
        console.log('%c[TRSCAN] installed â€” bounded heap scan for live TrophyRoadScreen (toggle off: localStorage.TRSCAN=0)','color:#0bf');
      ...
      // on a fresh boot (set only after purchases) â€” so FinishedDataFetch never fires â†’ splash hangs.
      // This probe writes Connector.FetchPlayerDataOnNextLoad=true continuously so Connector.Start calls
      // GetPlayerFullData on boot â†’ fires FinishedDataFetch â†’ should flip _didGetPlayerInfo. PURPOSE:
      ...
      // â€” but +0x6d is REALLY _didGetChallengesInfo (Ghidra setter BJLDJGDKLGNâ†’+0x6d, il2cpp.h confirmed).
      // The real boot blocker was challenges (fixed via are_challenges_enabled=true), NOT player-data.
      // Forcing FetchPlayerDataOnNextLoad=true on a guest boot fetches full player data â†’ makes the menu
      ...
      if (window._FORCE_FPDL === true) try {
        ...
        console.log('%c[FPDL-FORCE] installed â€” forcing Connector.FetchPlayerDataOnNextLoad=true (test)', 'color:#f0f');
      ...
      // state â€” must stay OFF. Now opt-in only.
      if (window._FIRE_FB4 === true) try {
        setTimeout(function(){
          try {
            ...
            // SELF-VERIFY: reset +0x6c (_didInitFirebase, â† OnUserAuthorized=FB+0x14 per il2cpp map) to 0.
            // If firing FB+0x14 flips it back to 1 â†’ the invoke mechanism WORKS (so _didGetPlayerInfo simply
            // is NOT an FB static event). If it stays 0 â†’ our invoke is a no-op (wrong call convention).
            h8[lsm+0x6c] = 0;
            ...
            // index of each LSM _did setter â†’ definitively map which event carries BJLDJGDKLGN(+0x6d).
            var TARGETS = {107483:'PNLDCHECEMN+0x6c(InitFb)',107484:'â˜…BJLDJGDKLGN+0x6d(GetPlayerInfo)',107485:'CCNOLPPJFJA+0x6f(AvailRegions)',
            ...
            // Directly invoke ONE closed delegate (entry) â€” its invoke_impl@+0xc with (entry) runs
            ...
            for (var si=0; si<SRC.length; si++){
              ...
              // â˜… RUNTIME IDX SHIFT = +1 (confirmed by DIAG scan: tbl[107485] sets +0x6d). The delegate's
              ...
              if (isPlayerInfo) {
                console.log('%c[FIRE-FB] â˜…â˜…â˜… FOUND: _didGetPlayerInfo(BJLDJGDKLGN) is subscribed to '+lbl+' â˜…â˜…â˜…','color:#0f8;font-weight:bold;font-size:16px');
                ...
                // m_target=loopLSM, method) â€” bypassing C# multicast iteration. DECISIVE: if +0x6d flips â†’
                // entry valid, multicast invoke is broken; if not â†’ entry dead/wrong-target. Opt-in.
                if (window.localStorage && localStorage.getItem('DIAG_CALL_BJ')==='1') {
                  ...
                  // â†’ runtime table idx != script.json idx. Scan a window of idxs, call each on loopLSM, find
                  // which one flips +0x6d (and log which byte 0x68-0x75 each touches). Establishes the shift.
                  console.log('%c[FIRE-FB] â˜… DIAG idx-shift scan (loopLSM=0x'+lsm.toString(16)+'): finding runtime idx that sets +0x6d (Ghidra=107484)','color:#fc0;font-weight:bold');
                  for (var ti=107478; ti<=107494; ti++){
                    ...
                    var changed=[]; for (var bb2=0x68; bb2<=0x75; bb2++){ if (h8[lsm+bb2]!==snap[bb2-0x68]) changed.push('0x'+bb2.toString(16)+'('+snap[bb2-0x68]+'â†’'+h8[lsm+bb2]+')'); }
                    if (changed.length) console.log('%c[FIRE-FB]   idx '+ti+' (Ghidra '+(ti-1)+'..'+ti+') â†’ changed bytes: '+changed.join(', '),'color:#0bf');
                    if (h8[lsm+0x6d]===1){ console.log('%c[FIRE-FB] â˜…â˜…â˜… idx '+ti+' SETS +0x6d (_didGetPlayerInfo)! shift vs Ghidra-107484 = '+(ti-107484)+' â˜…â˜…â˜…','color:#0f8;font-weight:bold;font-size:15px'); break; }
                  }
                }
              }
            }
            console.log('%c[FIRE-FB] walk done. (if no â˜…FOUND: BJLDJGDKLGN is on a class not in SRC â€” widen search)', 'color:#fc0;font-weight:bold');
          } catch(e){ console.warn('[FIRE-FB] err', e); }
        }, 13000);
        console.log('%c[FIRE-FB] armed â€” will fire FB events at 13s to find _didGetPlayerInfo source (webgl-native)', 'color:#0ff');
      ...
      if (window._FB8TL === true) try {   // DISABLED 2026-06-01: read-only but noisy + built on the disproven FB+8/"+0x6d=GetPlayerInfo" off-by-one premise. Opt-in only.
        ...
        var fb8iv = setInterval(function(){
          try {
            ...
            // If coro!=0 â†’ HDCJGIENLPB ran (CheckIfConnected continuation executed). If 0 â†’ it stalled after MKOFLHJKEML.
            ...
            if (d6d===1 && !window.__d6dFlipped){ window.__d6dFlipped=true;
              console.log('%c[FB8-TL] â˜…â˜…â˜… +0x6d FLIPPED to 1 at t='+Math.round(performance.now())+'ms (hidden='+document.hidden+') â€” FB+8 finally delivered / flag set â˜…â˜…â˜…','color:#0f8;font-weight:bold;font-size:15px'); }
            ...
            if (++fb8n > 240) clearInterval(fb8iv);   // ~6 min @1.5s â€” long enough to capture minimizeâ†’restore
          } catch(e){}
        }, 1500);
      } catch(e){}
      ...
      // out to be a CONFOUND â€” it breaks Addressables completion and hangs the boot even with
      ...
      if (window._ADDR_HOOK_ENABLE) try {
        ...
        // wasm module (only needs WebAssembly.Module/Instance â€” always available). The
        ...
        if (!tbl) {
          console.warn('%c[ADDR_HOOK] unavailable â€” no wasm Table found (candidates=[' + cand.join(',') + '])', 'color:#f80');
          ...
          // LIVE heap views â€” Unity swaps Module.HEAP32 on memory growth, so re-fetch each call
          // (captured views become detached/stale after growth â†’ all reads return undefined).
          ...
          // 2026-05-28: providers (Provide/Start) are the critical bundle-load path â€” wrapping
          // them broke boot. Try LoadAssetAsync-only (higher level) â€” may survive boot and still
      ...
      //     +8..+104 â€” pointers/floats/coroutines/etc (skipped)
      ...
      try {
        var STATIC_PTR = 10153368;       // 0x9AF358 â€” LoadingScreenManager static class metadata
        ...
        var probeInterval = setInterval(function() {
          try {
            ...
            // Stop after 12 snapshots (~60s) â€” long enough to observe all flag transitions in baseline.
            if (snapshotCount >= 12) {
              ...
              console.log('%c[LSM] FINAL @' + now + 'ms â€” DONE: ' + (doneJobs.length||'none') +
                          (doneJobs.length ? ' (' + doneJobs.join(', ') + ')' : '') +
                          ' â€” STUCK: ' + (stuckJobs.length||'none') +
                          (stuckJobs.length ? ' (' + stuckJobs.join(', ') + ')' : ''),
                          'color:#0f8;font-weight:bold');
            }
          } catch (e) { /* ignore â€” heap may be in odd state */ }
        }, 100);
      } catch (e) { console.warn('[LSM] probe setup failed:', e); }
      } // end if(false) â€” LSM probe disabled 2026-05-26
      ...
      // â˜…2026-06-10 PERF: FMOD audio spam (failed createDSP/addDSP per weapon-pickup) tanked the main thread.
      // â˜…ROOT: emscripten routes it via `console.WARN` (framework: var err=Module.printErr||console.warn.bind),
      // not console.error â€” so an error-only filter missed it. Throttle FMOD on ALL THREE console methods
      ...
      function _isFmodSpam(args) {
        ...
        // â˜…2026-06-12: FS.syncfs in-flight warnings â€” each renders a multi-thousand-frame async rAF stack
        // with DevTools open (~288k log lines/session) â†’ in-match jank. Same throttle (SYNCFSLOG=1 restores).
